...
export function Pricing() {
  ...
  const price = useMemo(() => {
    ...
    if (brandingEnabled) {
      total += BRANDING_BASE;
      if (posts > BRANDING_BASE_POSTS) {
        total += (posts - BRANDING_BASE_POSTS) * BRANDING_PER_POST;
      }
      if (engagement) {
        total += ENGAGEMENT_COST;
      }
    }

    if (leadgenEnabled) {
      total += LEADGEN_BASE;
      if (leads > LEADGEN_BASE_LEADS) {
        total += ((leads - LEADGEN_BASE_LEADS) / 1000) * LEADGEN_PER_1K;
      }
    }
      ...
      <div className="container-wide">
        <div className="max-w-2xl mb-16">
          ...
          <h2 className="font-serif text-heading-1 text-warm-900 mb-6">
            Build your plan<span className="text-accent">.</span>
          </h2>
          <p className="text-body-lg text-warm-500">
            No hidden fees, no "contact us for pricing." Pick what you need,
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              >
                <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${
                  brandingEnabled ? 'border-accent bg-accent' : 'border-warm-300'
                }`}>
                  {brandingEnabled && (
              ...
              >
                <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${
                  leadgenEnabled ? 'border-accent bg-accent' : 'border-warm-300'
                }`}>
                  {leadgenEnabled && (
                    <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                      <path d="M2 6l3 3 5-5" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              ...
              <div className="bg-white rounded-2xl p-8 border border-cream-300/60 shadow-sm space-y-8">
                <div>
                  <div className="flex justify-between items-baseline mb-4">
                    <label className="text-body font-medium text-warm-900">
                      Posts per month
                    </label>
                    <span className="font-serif text-heading-3 text-accent">{posts}</span>
                  </div>
                  <input
                    type="range"
                    min={10}
                ...
                <div className="border-t border-cream-200 pt-6">
                  <button
                    onClick={() => setEngagement(!engagement)}
                    className="flex items-center gap-3 group"
                  >
                    <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${
                      engagement ? 'border-accent bg-accent' : 'border-warm-300 group-hover:border-warm-400'
                    }`}>
                      {engagement && (
                        <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                          <path d="M2 6l3 3 5-5" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                    ...
                    <div className="text-left">
                      <p className="text-body font-medium text-warm-800">
                        Add engagement management
                      </p>
            ...
            {leadgenEnabled && (
              <div className="bg-white rounded-2xl p-8 border border-cream-300/60 shadow-sm">
                <div className="flex justify-between items-baseline mb-4">
                  <label className="text-body font-medium text-warm-900">
                    Prospects reached per month
                  </label>
                  <span className="font-serif text-heading-3 text-accent">
          ...
          <div className="lg:sticky lg:top-28">
            <div className="bg-warm-900 text-cream-50 rounded-2xl p-8">
              <p className="text-body-sm text-cream-400 uppercase tracking-wide mb-2">
                Your monthly investment
              </p>
              <div className="flex items-baseline gap-1 mb-8">
                <span className="font-serif text-display text-cream-50">
                  ${price.toLocaleString()}
                </span>
                <span className="text-body-sm text-cream-400">/mo</span>
              </div>

              {/* Breakdown */}
              <div className="space-y-3 mb-8 border-t border-warm-700 pt-6">
                {brandingEnabled && (
                  <>
                    <div className="flex justify-between text-body-sm">
                      <span className="text-cream-300">Personal Branding ({posts} posts)</span>
                    ...
                    {engagement && (
                      <div className="flex justify-between text-body-sm">
                        <span className="text-cream-300">Engagement management</span>
                        <span className="text-cream-100">${ENGAGEMENT_COST}</span>
                      </div>
                    )}
                  </>
                )}
                {leadgenEnabled && (
                  <div className="flex justify-between text-body-sm">
                    <span className="text-cream-300">Demand Gen ({leads.toLocaleString()} leads)</span>
                    <span className="text-cream-100">
                ...
                {!brandingEnabled && !leadgenEnabled && (
                  <p className="text-body-sm text-cream-400 italic">
              ...
              <a
                href="#cta"
                className="block w-full text-center bg-accent text-cream-50 font-medium py-3.5 rounded-full hover:bg-accent-dark transition-colors"
              >
                Get started
              </a>
            </div>
